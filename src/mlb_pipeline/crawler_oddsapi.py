# src/mlb_pipeline/crawler_oddsapi.py
...
# MLB off-season: no games, skip to save credits
    ...
    with conn.cursor() as cur:
        cur.execute("""
            ...
            WHERE provider = 'oddsapi'
              AND endpoint IN ('mlb_odds', 'mlb_odds_historical')
        """)
        row = cur.fetchone()
...
def _fetch_historical_day(cfg: OddsCrawlerConfig, conn, et_day: date) -> int | None:
    ...
    _save_payload(conn, endpoint="mlb_odds_historical", as_of_date=et_day,
...
def _fetch_live_day(cfg: OddsCrawlerConfig, conn, et_day: date) -> int | None:
    ...
    _save_payload(conn, endpoint="mlb_odds", as_of_date=et_day,
                  url=full_url, payload=payload, fetched_at_utc=fetched_at_utc)
    return credits_remaining
...
def _fetch_prop_lines_for_day(
    ...
    for ev in events:
        ...
        _save_payload(conn, endpoint="mlb_prop_odds", as_of_date=as_of_date,
                      url=full_url, payload=payload, fetched_at_utc=fetched_at_utc)
...
def main() -> None:
    ...
    parser.add_argument(
        "--force-props",
        action="store_true",
        help="Refresh today's prop event URLs even when they were fetched earlier.",
    )
