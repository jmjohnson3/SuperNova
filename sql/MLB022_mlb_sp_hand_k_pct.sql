-- MLB022: SP K% vs LHB and RHB separately
-- Plain VIEW — LATERAL-joined in train/predict. Leakage-safe: ROWS BETWEEN ... AND 1 PRECEDING.
DROP VIEW IF EXISTS features.mlb_sp_hand_k_pct CASCADE;
CREATE VIEW features.mlb_sp_hand_k_pct AS
WITH game_hand_stats AS (
    SELECT
        ...
        SUM(bgl.strikeouts_batter) FILTER (WHERE bh.bat_side = 'L')        AS k_vs_lhb,
        SUM(bgl.at_bats)           FILTER (WHERE bh.bat_side = 'R')        AS ab_vs_rhb,
        SUM(bgl.strikeouts_batter) FILTER (WHERE bh.bat_side = 'R')        AS k_vs_rhb
...
SELECT
    ...
    SUM(k_vs_lhb) OVER w25 / NULLIF(SUM(ab_vs_lhb) OVER w25, 0)  AS sp_k_pct_vs_lhb_25,
    SUM(k_vs_rhb) OVER w25 / NULLIF(SUM(ab_vs_rhb) OVER w25, 0)  AS sp_k_pct_vs_rhb_25,
    SUM(k_vs_lhb) OVER w10 / NULLIF(SUM(ab_vs_lhb) OVER w10, 0)  AS sp_k_pct_vs_lhb_10,
    SUM(k_vs_rhb) OVER w10 / NULLIF(SUM(ab_vs_rhb) OVER w10, 0)  AS sp_k_pct_vs_rhb_10,
